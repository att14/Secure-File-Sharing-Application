<?php
	...
	$_SESSION['prof_id'] = $_POST['prof_name'];

	$res = $db->prepare("SELECT name FROM professor WHERE p_id=" . $_SESSION['prof_id']);
	$res->execute();
	$res->bind_result($prof_name);

	while($res->fetch());

	$res = $db->prepare("SELECT AVG(overall), AVG(class_easiness), AVG(prof_easiness), AVG(prof_helpfulness) FROM rating, class WHERE rating.class_id=class.class_id AND p_id=" . $_SESSION['prof_id']);
	$res->execute();
	...
	$res = $db->prepare("SELECT AVG(overall), AVG(class_easiness), AVG(prof_easiness), AVG(prof_helpfulness) FROM rating, friends, class WHERE rating.class_id=class.class_id AND friends.fb_id=rating.fb_id AND p_id=" . $_SESSION['prof_id']);
	$res->execute();
	...
	echo '</tr>'; 
	echo '<tr>'; 
	echo '<td>Professor Helpfulness</td>';
	echo '<td>' . $avg_prof_helpfulness . '</td>';
	echo '<td>' . $f_avg_prof_helpfulness . '</td>';
	echo '</tr>'; 
	echo '</table><br />';

	$res = $db->prepare("SELECT fb_fname, fb_lname, season, year, course_code, course_name, review_timestamp, comments FROM appUser, course, rating, term, class WHERE rating.fb_id=appUser.fb_id AND term.term_id=class.term_id AND rating.class_id=class.class_id AND course.course_id=class.course_id AND p_id=" . $_SESSION['prof_id'] . " ORDER BY class.class_id DESC, review_timestamp DESC");
	$res->execute();
	$res->bind_result($fb_fname, $fb_lname, $season, $year, $course_code, $course_name, $review_timestamp, $comment);

	while ($res->fetch()) {
		echo $fb_fname . ' ' . $fb_lname . ' commented:<br />';
		echo $comment . '<br />';
		echo $course_code . ' ' . $course_name . ' during ' . $season . ' ' . $year . ' @ ' . $review_timestamp . '<br /><br />';
	}
